<x-guest-layout>
    @php
        $totalEquipment = $heavyEquipments->count();
        ...
        $readyPercent = $totalEquipment > 0 ? number_format(($readyCount / $totalEquipment) * 100, 1) : 0;
        $operatingPercent = $totalEquipment > 0 ? number_format(($operatingCount / $totalEquipment) * 100, 1) : 0;
        $maintenancePercent = $totalEquipment > 0 ? number_format(($maintenanceCount / $totalEquipment) * 100, 1) : 0;
        $damagedPercent = $totalEquipment > 0 ? number_format(($damagedCount / $totalEquipment) * 100, 1) : 0;
    @endphp
            ...
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="w-full mb-6">
                        ...
                        <div id="map" style="width: 100%; height: 400px;" class="rounded-lg border z-10 border-gray-300"></div>
                    </div>
                    ...
                    <div class="flex flex-row gap-6 mb-6">
                        ...
                        <div class="w-1/2">
                            <div class="bg-gray-100 shadow-md p-4 rounded-lg mb-4">
                                <h3 class="font-semibold mb-2 text-yellow"><span class="bg-yellow-200 text-yellow-800 px-2 py-1 rounded-full">Sedang Bertugas</span></h3>
                                    ...
                                    <table class="table-auto w-full">
                                       ...
                                       <tbody>
                                            @forelse($ongoingProjectsMap as $project)
                                                <tr class="border-t border-gray-300 hover:bg-gray-50 transition duration-150 ease-in-out">
                                                    <td class="px-4 py-2">{{  $project['heavy_equipment']['nomor_lambung'] }}</td>
                                                    <td class="px-4 py-2">
                                                ...
                                                <tr>
                                                    <td colspan="3" class="px-6 py-8 text-center">
                                                        <div class="flex flex-col items-center justify-center">
                                                            ...
                                                            <span class="text-gray-500 text-sm">Tidak ada pekerjaan yang sedang berlangsung saat ini</span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforelse
                        ...
                        <div class="flex-1">
                            <div class="bg-gray-100 shadow-md p-4 rounded-lg mb-4 w-full">
                                <h3 class="font-semibold mb-2 text-green"><span class="bg-green-200 text-green-800 px-2 py-1 rounded-full">Tersedia</span></h3>
                                <div class="overflow-y-auto max-h-[255px]">
                                    <table class="table-auto w-full">
                                        <thead>
                                            <tr class="bg-gray-200 sticky top-0 z-10">
                                                ...
                                                <td class="px-4 py-2">
                                                    <span class="rounded-full text-md font-semibold px-2 py-1
                                                        {{ $equipment->status == 'ready' ? 'bg-green-200 text-green-800' :
                                                            ...
                                                            ($equipment->status == 'maintenance' ? 'bg-orange-200 text-orange-800' :
                                                            ($equipment->status == 'rusak' ? 'bg-red-200 text-red-800' : ''))) }}">
                            ...
                            <div class="bg-gray-100 shadow-md p-4 rounded-lg">
                                ...
                                <div class="flex flex-col items-center">
                                    ...
                                    <div class="text-sm">
                                        ...
                                        <p>Ready: {{ $heavyEquipments->where('status', 'ready')->count() }}
                                            ({{ number_format(($heavyEquipments->where('status', 'ready')->count() / $heavyEquipments->count()) * 100, 1) }}%)
                                        </p>
                                        <p>Beroperasi: {{ $heavyEquipments->where('status', 'beroperasi')->count() }}
                                            ({{ number_format(($heavyEquipments->where('status', 'beroperasi')->count() / $heavyEquipments->count()) * 100, 1) }}%)
                                        </p>
                                        <p>Maintenance: {{ $heavyEquipments->where('status', 'maintenance')->count() }}
                                            ({{ number_format(($heavyEquipments->where('status', 'maintenance')->count() / $heavyEquipments->count()) * 100, 1) }}%)
                                        </p>
                                        <p>Rusak: {{ $heavyEquipments->where('status', 'rusak')->count() }}
                                            ({{ number_format(($heavyEquipments->where('status', 'rusak')->count() / $heavyEquipments->count()) * 100, 1) }}%)
                                        </p>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            ...
            `;
                        ...
                        locationButton.onAdd = function(map) {
                            ...
                            div.innerHTML =
                                '<a href="#" title="Lokasi Saya" role="button" aria-label="Lokasi Saya" onclick="addUserLocation(); return false;"><span aria-hidden="true">📍</span></a>';
                            return div;
                        };
